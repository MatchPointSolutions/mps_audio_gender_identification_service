...
import numpy as np
import pandas as pd
from config import DATASET,CHILD_MODEL_FILENAME
...
def train_model(df):
    ...
    logger.info(len(df))
    df['features'] = df['filename'].apply(lambda x: extract_features_1(os.path.join("./dataset_path", x)))
    logger.info(df.head(10))
    df['gender_encoded'] = df['gender'].map({'adult': 0, 'child': 1})
    ...
    logger.info(report)
    joblib.dump(model, CHILD_MODEL_FILENAME)
    logger.info(f"Model saved to {CHILD_MODEL_FILENAME}")

# Train the Model
# train_model(df)

def identify_the_child_audio(input_file,model_filename=CHILD_MODEL_FILENAME):
    """
    ...
    logger.info(f"Predicted gender: {'child' if prediction[0] == 1 else 'adult'}")
    if prediction[0] == 1:
        return "child"
    else:
        return "adult"
